...
| ARM BLE guest (M0+/M33, bring-up stalls) | 🟡 | `test-firmware/gen_ble_arm.py` → `ble_adv.S` → `web/ble_adv{,_pico2}.uf2` (PIO0 SM0, `ARM BLE LISTEN` target mirroring `wifi_ble_adv_rv32.uf2`): builds clean, runs to `ARM BLE Starting`, first gSPI transfer still decodes `addr=0 size=0` — r1-preservation + restart/skip-word fixes applied, window encode still under test. |
...
| What is NOT done | 🟡 | Items above; MP live `gap_advertise` still needs a BT-capable MP build whose ADV stays observable (see BLE row). |
