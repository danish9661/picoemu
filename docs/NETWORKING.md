...
| Arduino ioLibrary guest (M0+ ✅, M33 SDK-async gap) | 🟡 (M33 pump) | `test-firmware/arduino/ethdhcp/ethdhcp.ino` (`Wiznet5500lwIP`, `rp2040:rp2040:wiznet_5500_evb_pico`): full DORA green since the RX cursor-latch fix (first DATA byte of each CS frame latched the stale base 0, corrupting nonzero-address RX bursts: OFFER head at RX_RD=0 worked, ACK head at RX_RD=0x0158 read hi=0x00 not 0x01, len 342→86, ACK desynced — latch now happens BEFORE the read; peer `ALL DHCP CHECKS PASSED` + guest `conn=1 ip=192.168.4.2`). M33 (`ethdhcp_m33`, `Serial1`, `usbstack=nousb` to skip the unmodeled-USB wait): prints START then stalls before `eth.begin()` returns — NOT an M33 defect (stock-SDK `m33probe` millis/TICKS green; in-tree `eth_dhcp_pico2` DORA green) but the SDK `async_context_threadsafe_background` pump: `m33alarm` shows alarm-pool hardware-IRQ callbacks never fire (arch-independent — same on M0+, so lwIP's 20ms `ethernet_timeout_reached` never runs `handlePackets()` on EITHER arch; M0+ only binds because its DHCP path completes synchronously enough). Next: model the hardware-alarm IRQ delivery (TIMER IRQ 0-7 via NVIC) or poll the async-context workers from the main loop. |
...
| What is NOT done | 🟡 | Arduino ioLibrary M33 E2E (SDK `async_context_threadsafe_background` pump: alarm-pool hardware-IRQ callbacks never fire, arch-independent — lwIP 20ms poll never runs `handlePackets()`); MP live `gap_advertise`: stock Pico-W MP boots to REPL but `import bluetooth` hangs the guest (BT stack never reaches `BLE-ACTIVE`; needs deeper HCI bring-up work — see CHANGELOG). |
