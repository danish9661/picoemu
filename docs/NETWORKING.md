...
| What is NOT done | 🟡 | Arduino ioLibrary E2E post-OFFER stall (guest-side RX pump under test — emulator RX bytes verified correct via unit shim); MP live `gap_advertise`: stock Pico-W MP boots to REPL but `import bluetooth` hangs the guest (BT stack never reaches `BLE-ACTIVE`; needs deeper HCI bring-up work — see CHANGELOG). |
